...
class BN(object):
    ...
    def analyze_attractor_structure(self, start_state=None, max_steps=10000):
        """
        ULEPSZONA WERSJA: Przeszukuje całą przestrzeń stanów (2^N).
        Znajduje WSZYSTKIE unikalne atraktory w sieci.

        Zwraca:
            all_transients (list): Lista ścieżek prowadzących do atraktorów.
            all_attractors (list): Lista unikalnych cykli (atraktorów).
            Indeksy obu list sobie odpowiadają (i-ta ścieżka prowadzi do i-tego atraktora).
        """
        all_transients = []
        all_attractors = []

        # Zbiór wszystkich stanów, które już kiedykolwiek odwiedziliśmy w symulacji.
        # Dzięki temu unikamy robienia tej samej roboty dwa razy.
        visited_global = set()

        # Iterujemy po wszystkich możliwych stanach od 00...0 do 11...1
        for i in range(2 ** self.num_nodes):
            state = self.__int_to_state(i)

            # Jeśli ten stan już badaliśmy przy okazji innej ścieżki -> pomijamy
            if state in visited_global:
                continue

            # --- Rozpoczynamy nową symulację ---
            history = []
            seen_local = {}  # Służy do wykrycia cyklu w TEJ konkretnej ścieżce
            curr = state

            steps = 0
            # Idziemy dopóki:
            # 1. Nie wpadniemy w globalnie znany teren (stary atraktor)
            # 2. Nie zapętlimy się lokalnie (nowy atraktor)
            while (curr not in visited_global) and (curr not in seen_local) and (steps < max_steps):
                seen_local[curr] = len(history)
                history.append(curr)
                visited_global.add(curr)  # Oznaczamy, że tu już byliśmy

                curr = self.get_next_state_synchronous(curr)
                steps += 1

            # Analiza wyniku symulacji:
            if curr in seen_local:
                # Sukces! Trafiliśmy w ogon obecnego węża -> Mamy NOWY, unikalny cykl.
                start_index = seen_local[curr]

                transient_part = history[:start_index]
                attractor_part = history[start_index:]

                all_transients.append(transient_part)
                all_attractors.append(attractor_part)

            # Jeśli pętla przerwała się, bo 'curr in visited_global',
            # to znaczy, że ścieżka wpadła do starego basenu atrakcji.
            # Nie dodajemy jej, żeby nie dublować atraktorów w wynikach.

        return all_transients, all_attractors
...
if __name__ == "__main__":
    ...
    for N in sizes_to_test:
        ...
        # Pobieramy wszystkie, bo funkcja tak teraz działa
        all_transients, all_attractors = bn.analyze_attractor_structure()

        # Sprawdzamy, czy cokolwiek znaleziono (zawsze powinno, ale dla bezpieczeństwa)
        if all_attractors:
            # BIERZEMY TYLKO PIERWSZY WYNIK z listy, aby mieć "Single Trajectory"
            s_trans = all_transients[0]
            s_attr = all_attractors[0]

            s_full = s_trans + s_attr

            # Teraz zapisywanie zadziała poprawnie (bo s_full to lista stanów, a nie lista list)
            save_to_file(s_full, names, "{}/data_size{}_sync_SINGLE_full.txt".format(output_dir, N))
            save_to_file(s_trans + s_attr[:3], names,
                         "{}/data_size{}_sync_SINGLE_trans_heavy.txt".format(output_dir, N))
            save_to_file(s_trans[-3:] + s_attr, names,
                         "{}/data_size{}_sync_SINGLE_attr_heavy.txt".format(output_dir, N))
        ...
        s_raw_async = bn.simulate_trajectory_async(start_node_single, steps=1000)
        save_to_file(s_raw_async, names, "{}/data_size{}_async_SINGLE_full.txt".format(output_dir, N))
        save_to_file(s_raw_async[:50], names, "{}/data_size{}_async_SINGLE_trans_heavy.txt".format(output_dir, N))
        save_to_file(s_raw_async[-50:], names, "{}/data_size{}_async_SINGLE_attr_heavy.txt".format(output_dir, N))
        ...
        # in this section datasets containing only multiple (25 = NUM_STARTS) trajectories are generated
        # for both synchronous and asynchronous mode we generate 4 datasets:
        # entire trajectory, only the beginning (transient + small part of attractor),
        # only the end (small part of transient + full attractor), entire trajectory sampled every 3rd step
        ...
        # --- NOWA LOGIKA: Zamiast losować, pobieramy kompletny obraz sieci ---
        print("  > Skanowanie atraktorów...")
        # Argument start_state jest ignorowany, ale podajemy go dla zgodności
        start_dummy = tuple([0] * N)
        found_transients, found_attractors = bn.analyze_attractor_structure(start_dummy)

        print("  > Znaleziono {} unikalnych atraktorów.".format(len(found_attractors)))

        # Iterujemy przez WSZYSTKIE znalezione atraktory
        for trans, attr in zip(found_transients, found_attractors):
            full_traj = trans + attr
            ...
            sync_trans_heavy.extend(trans + attr_start)
        ...
        for _ in range(NUM_STARTS):
            ...
            # simulating 1000 asynchronous steps
            raw_traj = bn.simulate_trajectory_async(start_node, steps=1000, sampling_freq=1)
            ...
            async_trans_heavy.extend(raw_traj[:50])

            # Dataset 3: taking 50 last observations. It represents settled system - it should contain settle in an attractor.
            async_attr_heavy.extend(raw_traj[-50:])
